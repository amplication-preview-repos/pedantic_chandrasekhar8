/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { HealthcareProviderWhereUniqueInput } from "./HealthcareProviderWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { HealthcareProviderUpdateInput } from "./HealthcareProviderUpdateInput";

@ArgsType()
class UpdateHealthcareProviderArgs {
  @ApiProperty({
    required: true,
    type: () => HealthcareProviderWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => HealthcareProviderWhereUniqueInput)
  @Field(() => HealthcareProviderWhereUniqueInput, { nullable: false })
  where!: HealthcareProviderWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => HealthcareProviderUpdateInput,
  })
  @ValidateNested()
  @Type(() => HealthcareProviderUpdateInput)
  @Field(() => HealthcareProviderUpdateInput, { nullable: false })
  data!: HealthcareProviderUpdateInput;
}

export { UpdateHealthcareProviderArgs as UpdateHealthcareProviderArgs };
